...
def lis(a):
    ...
    for i in xrange(len(a)):
        print "Before operations Tree:",T
        m = a[i]
        T.add(m)
        print "Cur:",m,"predecessor:",T.predecessor(m),"Tree:",T
        if T.predecessor(m):
            L[m] = L[T.predecessor(m)]+1
        else:
            L[m] = 1
        if T.successor(m):
            if L[T.successor(m)] == L[m]:
                print "discarding:",T.successor(m)
                T.discard(T.successor(m))
        print "After operations Tree:",T
    print T
    print L
    return max(L.values())
