...
bool solve(vector<int> store[], vector<int>& final, int n, int i){
	// cout << "i: " << i << " => "; Print(store, n);
	if(i == final.size()){
	...
	bool flag = false;
	for (int j = 0; j < n; j += 1){
		if(!store[j].empty() and store[j].back() == cur){
			store[j].pop_back();
			answers.psb(solve(store, final, n, i+1));
			store[j].psb(cur);
			flag = true;
		}
	}
	if (flag == false) return false;
	for (bool x: answers) if (x) return x;
	return false;
}
...
int main(void){
	...
	while(t--){
		...
		// cout << "final: ";print(final, final.size());
		if (solve(store, final, n, 0)) cout << "Yes" << endl;
		else cout << "No" << endl;
	}
}